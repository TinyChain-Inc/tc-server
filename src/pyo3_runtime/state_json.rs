...
async fn encode_btree_state_json(btree: BTreeCollection) -> Result<Vec<u8>, String> {
    ...
    let payload = encode_btree_payload_json(btree.schema, btree.btree).await?;
...
async fn encode_btree_payload_json(
    ...
    btree: tc_collection::btree::BTree,
...
pub(super) async fn encode_state_json_bytes(state: State) -> Result<Vec<u8>, String> {
    match state {
