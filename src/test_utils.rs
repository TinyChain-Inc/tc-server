...
pub(crate) fn wasm_echo_request_module() -> Vec<u8> {
    static BYTES: OnceLock<Vec<u8>> = OnceLock::new();
    BYTES.get_or_init(build_wasm_echo_request_module).clone()
}
...
fn build_wasm_echo_request_module() -> Vec<u8> {
    const SCHEMA_ID: &str = "/lib/example-devco/example/0.1.0";
    const MANIFEST_OFFSET: u32 = 8;
    const HEAP_INIT: u32 = 1024;
    ...
    // hello(header_ptr, header_len, body_ptr, body_len) -> i64
    // Allocate a response buffer, copy the request body, then return (len<<32 | ptr).
    let mut hello = Function::new([(1, ValType::I32)]);
    hello.instruction(&Instruction::LocalGet(3));
    hello.instruction(&Instruction::Call(0));
    hello.instruction(&Instruction::LocalSet(4));
    hello.instruction(&Instruction::LocalGet(4));
    hello.instruction(&Instruction::LocalGet(2));
    hello.instruction(&Instruction::LocalGet(3));
    hello.instruction(&Instruction::MemoryCopy {
        src_mem: 0,
        dst_mem: 0,
